...
export class SessionManager {
   ...
   * 记录原始会话到成功会话的映射
   *
   * 用于会话复用场景：当一个原始会话经过处理（可能因并发检测、故障转移等原因创建了新sessionId）
   * 最终成功完成后，记录这个映射关系，下次客户端使用同一个原始会话时可以直接复用成功的sessionId
   *
   * 好处：
   * 1. 避免同一原始会话每次都创建新的sessionId
   * 2. 避免频繁切换provider
   * 3. 保持会话的连续性和稳定性
   *
   * @param originalSessionId - 原始会话ID（客户端传递的）
   * @param successfulSessionId - 最终成功的会话ID（实际使用的）
   * @param userId - 用户ID（用于日志追踪）
   * @param providerId - 供应商ID（用于日志追踪）
   * @returns 是否成功记录
   */
  static async recordSuccessfulSessionMapping(
    originalSessionId: string,
    successfulSessionId: string,
    userId?: number,
    providerId?: number
  ): Promise<boolean> {
    ...
    const redis = getRedisClient();
    if (!redis || redis.status !== "ready") {
      logger.warn("SessionManager: Redis not ready, cannot record successful session mapping", {
        originalSessionId,
        successfulSessionId,
      });
      return false;
    ...
    try {
      // 映射关系 key: session_success_mapping:{originalSessionId}
      const mappingKey = `session_success_mapping:${originalSessionId}`;

      // 存储映射关系（保留 30 天，足够长期复用）
      const mappingData = {
        originalSessionId,
        successfulSessionId,
        userId,
      ...
      logger.info("SessionManager: Recorded successful session mapping", {
        originalSessionId,
        successfulSessionId,
        userId,
        providerId,
      ...
      logger.error("SessionManager: Failed to record successful session mapping", {
        error,
        originalSessionId,
        successfulSessionId,
      });
   ...
   * 查询原始会话的成功映射
   *
   * 用于会话复用场景：在处理新请求时，先检查该原始会话是否之前有成功的映射
   * 如果有，直接使用之前成功的sessionId，避免重复创建和频繁切换provider
   *
   * @param originalSessionId - 原始会话ID
   * @returns 成功的会话ID，如果不存在映射则返回 null
   */
  static async getSuccessfulSessionMapping(originalSessionId: string): Promise<string | null> {
    const redis = getRedisClient();
    if (!redis || redis.status !== "ready") {
      return null;
    }
    ...
    try {
      const mappingKey = `session_success_mapping:${originalSessionId}`;
      const mappingDataStr = await redis.get(mappingKey);

      if (!mappingDataStr) {
        return null;
      }

      const mappingData = JSON.parse(mappingDataStr) as {
        originalSessionId: string;
        successfulSessionId: string;
        userId?: number;
        providerId?: number;
      ...
      logger.debug("SessionManager: Found successful session mapping", {
        originalSessionId,
        successfulSessionId: mappingData.successfulSessionId,
        createdAt: mappingData.createdAt,
      });

      return mappingData.successfulSessionId;
    } catch (error) {
      logger.error("SessionManager: Failed to get successful session mapping", {
        error,
        originalSessionId,
      });
      return null;
