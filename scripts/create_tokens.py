    ...
    def batch_create(
        ...
        for i in range(1, count + 1):
            ...
            if result and result["success"]:
                ...
                print(f"  [\033[32mOK\033[0m] {token_name} -> {token_key}")
                tokens.append({"name": token_name, "key": token_key, "user_id": user_id})
                success_count += 1
            else:
   ...
   session=xxx;acw_tc=yyy|6702|mytoken|10
    ...
    if creator.created_tokens:
        ...
        keys_file.write_text(
            "\n".join(t["key"] for t in creator.created_tokens),
            encoding="utf-8",
        )
        print(f"Token keys已保存到: {keys_file}")
    else:
        print("\n未创建任何token")
